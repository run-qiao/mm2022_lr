...
def lof_fit(data,k=5,method='l2'):
    clf = LocalOutlierFactor(n_neighbors=k + 1, algorithm='auto', metric=method, contamination=0.1,novelty=True)
    clf.fit(data)
    return clf
def lof(predict, clf,k=5, method='l2',thresh=2):
    # 计算 LOF 离群因子
    predict = -clf.score_samples(predict)
    # predict=predict[200:]
    # 根据阈值划分离群点与正常点
...
class ATOM(BaseTracker):
    ...
    def __init__(self,params):
        super(ATOM,self).__init__(params)
