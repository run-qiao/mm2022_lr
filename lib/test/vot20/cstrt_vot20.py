...
class cstrt_vot20(object):
    def __init__(self, tracker_name='cstrt', para_name='baseline_cs'):
        # create tracker
...
def run_vot_exp(tracker_name="cstrt", para_name="baseline_cs", vis=False):
    torch.set_num_threads(1)
    save_root = os.path.join('/home/szk/Developer/vot2020_results/CSTrT/', para_name)
    if vis and (not os.path.exists(save_root)):
        os.mkdir(save_root)
    tracker = cstrt_vot20(tracker_name=tracker_name, para_name=para_name)
