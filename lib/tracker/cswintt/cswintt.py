...
from lib.models.cswintt_1 import build_cswintt
from lib.tracker.cswintt.cswintt_utils import Preprocessor
...
class CSWinTT(BaseTracker):
	def __init__(self, params, dataset_name):
		...
		# network.load_state_dict(torch.load(self.params.checkpoint_cls, map_location='cpu')['net'], strict=False)
		network.load_state_dict(torch.load(self.params.checkpoint_cls, map_location='cpu'), strict=True)
	...
	def track(self, image, info: dict = None):
		...
		# draw_image(image,self.state)
		# draw_image(x_patch_arr)
		# update template
